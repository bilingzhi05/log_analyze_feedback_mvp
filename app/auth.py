...
from .db import execute_query, fetch_one, fetch_value
...
def record_login_failure(connection, username: str, user_ip: str) -> int:
    """
    功能：记录登录失败次数并返回最新次数。
...
def reset_login_failures(connection, username: str, user_ip: str) -> None:
    """
    功能：清除登录失败次数记录。
    参数：connection（数据库连接）、username（用户名）、user_ip（IP 地址）。
    返回值：无。
...
def get_failure_count(connection, username: str, user_ip: str) -> int:
    """
    功能：获取登录失败次数。
    参数：connection（数据库连接）、username（用户名）、user_ip（IP 地址）。
    返回值：失败次数。
...
def create_captcha(connection) -> Tuple[str, str]:
    """
    功能：生成验证码并写入数据库。
    参数：connection（数据库连接）。
...
def verify_captcha(connection, captcha_id: str, captcha_code: str) -> bool:
    """
    功能：校验验证码是否有效。
...
def require_admin(request: Request, connection) -> str:
    """
    功能：校验管理员会话并返回用户名。
...
def clear_session(response: Response, connection, request: Request) -> None:
    """
